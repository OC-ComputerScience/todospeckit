...
async function upsertStoriesForFeature(feature, scopeRef, epicOid, { previewOnly = false } = {}) {
...
function printDryRun(backlog, scopeRef, featureOnly) {
  ...
  if (featureOnly) {
    console.log(
      upsertMode
        ? "Mode: feature upsert (use live credentials for lookup plan; see --upsert without --dry-run)"
        : "Mode: feature push (stories + tests only)",
    );
    console.log("");
    for (const feature of backlog.features) {
      console.log(
